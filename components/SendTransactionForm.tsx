...
export default function SendTransactionForm() {
    ...
    // **Type Assertion Here**
    const usdtTokenAddress = USDT_TOKEN_ADDRESS as `0x${string}`;
    const pooledWalletAddress = POOLED_WALLET_ADDRESS as `0x${string}`;

    // Optionally, validate that the addresses start with '0x'
    if (!usdtTokenAddress.startsWith('0x')) {
        throw new Error('USDT_TOKEN_ADDRESS must start with "0x"');
    }
